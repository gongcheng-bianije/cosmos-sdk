...
func TestGaiaCLISend(t *testing.T) {
	...
	executeWrite(t, fmt.Sprintf("gaiacli send %v --amount=10steak --to=%s --name=foo", flags, barAddr), pass)
	tests.WaitForNextHeightTM(port)
	...
	// test memo
	executeWrite(t, fmt.Sprintf("gaiacli send %v --amount=10steak --to=%s --name=foo --memo 'testmemo'", flags, barAddr), pass)
	tests.WaitForNextHeightTM(port)
...
func TestGaiaCLISubmitProposal(t *testing.T) {
	...
	executeWrite(t, fmt.Sprintf("gaiacli gov submitproposal %v --proposer=%s --deposit=5steak --type=Text --title=Test --description=test --name=foo", flags, fooAddr), pass)
	tests.WaitForNextHeightTM(port)
	...
	executeWrite(t, fmt.Sprintf("gaiacli gov deposit %v --depositer=%s --deposit=10steak --proposalID=1 --name=foo", flags, fooAddr), pass)
	tests.WaitForNextHeightTM(port)
	...
	executeWrite(t, fmt.Sprintf("gaiacli gov vote %v --proposalID=1 --voter=%s --option=Yes --name=foo", flags, fooAddr), pass)
	tests.WaitForNextHeightTM(port)
